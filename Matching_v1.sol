...
contract Coupon{
   ...
   //存投資需求資訊
    ...
    modifier enoughToken(address investor ,uint amount){
        require(platform_token._balanceOf(investor)>=amount);
        _;
    }
    function make_invest_requirement(address investor,uint amount)public onlyOwner enoughToken(investor,amount)returns(bytes32 amount_,bytes32 investmentID){
        //investor=msg.sender;
...
contract Matching is Coupon,Credit{
    ...
    modifier enoughCredit(address addr_issue,address borrower,uint amount) {
	    Credit credit=Credit(addr_issue);
	    require(credit.getBalance(borrower)>=amount);
	_;
    }
  //創建資金需求
    function make_funding_requirement(address borrower,address addr_issue,uint amount,bytes32 creditID)public enoughCredit(addr_issue,borrower,amount)returns(bytes32 amount_,bytes32 requirementID){
      //address borrower=msg.sender;
      requirementID=keccak256(abi.encodePacked(borrower,amount,creditID));
      bytes32 borrower_=keccak256(abi.encodePacked(borrower));
      assembly { mstore(add(amount_,32),amount)}
  ...
  //媒合資金需求
    ...
    function distribute_funding(address addr_issue,address borrower,uint amount)internal returns(bool isdistributed){
        ...
        credit._transferFrom(borrower,platform,amount);
        platform_token._transferFrom(platform,borrower,amount);
        isdistributed=true;
